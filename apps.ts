...
let gameObj = {
    ...
    runLoop: async function(){
        ...
        while (running) {
           let  enemyHealth : number = Math.floor(Math.random() * this.enemiesHealth)
           ...
           while(enemyHealth > 0 ){
            ...
            let input = await  inquirer.prompt([{
                ...
                if(input.choices == "Attack"){
                    let damageDealt : number = Math.floor(Math.random()*this.attackDamage)
                    let damageTaken : number = Math.floor(Math.random()*this.enemyAttackDamage)
